...
**Current status:** The formal baseline and cost sweep are verified. The author manually deployed a public, running Hugging Face Static Space in the course organization. The individual-author ACM paper compiles locally with Figure 1 and two author-confirmed field-trip photos; its five numbered main sections finish on page 2. The final Overleaf compile and human/course-process inputs remain. Behavioral questions and educational effectiveness remain untested.

## Public research artifacts

- [Personal GitHub v2 branch](https://github.com/micL1222/PS1-Yiqiao/tree/v2-information-acquisition)
- [GitHub-backed Colab notebook](https://colab.research.google.com/github/micL1222/PS1-Yiqiao/blob/v2-information-acquisition/notebooks/information_acquisition_baseline.ipynb) (fresh-executed locally; hosted Colab runtime not independently executed)
- [Hugging Face Static Space](https://huggingface.co/spaces/dku-comsci-econ206-2026/who-pays-to-know) (public and running)
...
The deployed browser-side game logic in `deployment/huggingface-static/` implements the same formal payoff model as the trusted Python code. `scripts/validate_static_space.py` checks payoffs, pure equilibria, and mixed-strategy cases against Python for V = 4 and c = 0 through 5. The released tool illustrates the model; educational effectiveness has not been evaluated.
...
| `deployment/huggingface-static/` | Five-file source of the final deployed Static Space |
| `deployment/huggingface/` | Historical Gradio deployment staging, never deployed |
...
- Hugging Face Space: [the final Static Space](https://huggingface.co/spaces/dku-comsci-econ206-2026/who-pays-to-know) was manually uploaded by the author to the official course organization and is public and running. The earlier Gradio staging in `deployment/huggingface/` was not deployed; its hosting-plan failures are recorded in `docs/development_log.md`.
