...
Common thresholds will map either score to interrupt, checkpoint, or digest. $R$ will use labeled validity from matching task types, with smoothing for sparse histories. A synthetic simulation will vary validity, severity, arrival time, and agent history while comparing $S=N$, $S=NR$, and smoothed reliability.

Metrics are unnecessary interruptions, important-message delay, warnings missed by minute 20, attention cost, and authority share by agent. Checks cover bounds, monotonicity, seeded runs, threshold edges, and Python--Colab--demo agreement. No project simulation, deployment, safety validation, or real-agent result is claimed. Calibration would fail if it saves attention mainly by suppressing valid warnings.
...
Planned materials are transparent scheduler code, documented synthetic inputs, tests, a notebook, and saved outputs tied to a verified commit. No project-specific release or computational result is claimed yet; links and fresh-run instructions will be added after verification.

\subsection*{Statement of Contribution to the UN's SDGs}
\textbf{Hugging Face:} \DemoLink\\
The planned open-access simulator aims to support \textbf{SDG 4---Quality Education}~\cite{sdg4} by helping students compare current-claim-only and history-calibrated attention rules in an interactive scenario. Its accessibility, instructions, and learning evaluation remain to be developed; no educational benefit is claimed yet.
